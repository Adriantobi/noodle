...
import { Archive, ChevronLeft, ChevronRight, Heart, Instagram, LinkIcon, Twitter, Volume2, VolumeX, Youtube } from 'lucide-react'

export default function SpacesMenuWidget( { spaces, creator, categories, widgetState, menuState, setAllSpaceDetails, setIframeVolume } ) {
        //Close MenuSpaces
    ...
    useEffect(() => {
        window.addEventListener('click', () => setExpand(true))
        window.addEventListener('mousemove', () => setExpand(true))
    },[])
    ...
    useEffect(() => {
        ...
        if (muted === true) {
            ...
            if (tempVol > 0) {
                document.querySelector(`.${styles.volumeSlider}`).value = tempVol
                setVolume(tempVol)
            }
    ...
    return (
        <div className={`${styles.SpacesMenuWidgetWrapper}`} style={{display : menuState ? 'flex' : 'none'}}>
                ...
                <div className={styles.SpacesMenu}>
                    ...
                    <div className={styles.shufflerHeader}>
                        ...
                        <div className={styles.shufflerControls}>
                            <div className={styles.shufflerLeft} onClick={moveCarouselLeft}><ChevronLeft /></div>
                            <div className={styles.shufflerRight} onClick={moveCarouselRight}><ChevronRight /></div>
                        </div>
                    </div>
                    ...
                    <div className={styles.spacesInfoBlock}>
                        <div className={styles.spaceIndicator}>
                            ...
                            <div className={styles.spacesOptions}>
                                <div className={styles.saveSpace} onClick={ () => setSaved(!saved) }><Heart stroke='#d68e7f' fill={saved ? '#d68e7f' : 'none'}/></div>
                                <div className={styles.playlist}><Archive /></div>
                            </div>
                        </div>
                        <div className={styles.volumeSettingSection}>
                            ...
                            <input type="range" min="0" max="100" value={volume} className={styles.volumeSlider} onInput={(event) => setVolume(event.target.value)}/>
                        </div>
                    </div>
                    ...
                    <div className={styles.spaceCreatorInfo}>
                        ...
                        <div className={styles.creatorTextContent}>
                            <div className={styles.creatorUserNameSection}><span className={styles.creatorUserName}>@{creator.name}</span><sup><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 10 10" fill="none" class="feather feather-verified-icon"><path fill="#fff" d="M1.875 1.875h6.25v6.25h-6.25z"></path><path fill-rule="evenodd" d="M5 10a5 5 0 003.536-8.536 5 5 0 00-7.072 0A5 5 0 005 10zm2.317-5.808a.62.62 0 00.175-.44.63.63 0 00-.183-.437.624.624 0 00-.437-.183.63.63 0 00-.44.175L4.375 5.366l-.808-.808a.62.62 0 00-.44-.175.63.63 0 00-.437.183.624.624 0 00-.183.437.63.63 0 00.175.44l1.25 1.25a.625.625 0 00.884 0l2.5-2.5z" fill="#127fff"></path></svg></sup></div>
                            <div className={styles.creatorLinks}>
                                { creator.instagram ? <Link href={creator.instagram} target="_blank" id="instagram" className={styles.socialLinksForCreator}><Instagram /></Link> : null }
                                { creator.twitter ? <Link href={creator.twitter} target="_blank" id="twitter" className={styles.socialLinksForCreator}><Twitter /></Link> : null }
                                { creator.website ? <Link href={creator.website} target="_blank" id="website" className={styles.socialLinksForCreator}><LinkIcon /></Link> : null }
                                { creator.youtube ? <Link href={creator.youtube} target="_blank" id="youtube" className={styles.socialLinksForCreator}><Youtube /></Link> : null }
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={styles.spacesSideBarFooterButtons}>
                        <div className={`${styles.desktopApp} ${styles.noodleInfoButton}`}><Link href="#" target="_blank">Desktop App</Link></div>
                        <div className={`${styles.showCaseButton} ${styles.noodleInfoButton}`}><Link href="#" target="_blank">Showcase</Link></div>
                        <div className={`${styles.faqLinkButton} ${styles.noodleInfoButton}`}><Link href="#" target="_blank">FAQ</Link></div>
                    </div>
                </div>
            </div>
    )
