...
export default function DropMenu( { full, modalStates } ) {
    const { status } = useSession()
    
    return (
        <div className={`${styles.dropDownMenu} ${full ? '' : `${styles.top}` }`}>
            { status === 'authenticated' ?
                <span className={styles.dropDownMenuButton} onClick={() => modalStates('account')}>
                    <div>
                        <User />
                        My Account 
                    </div>
                </span> : 
                <span className={styles.dropDownMenuButton} onClick={() => signIn('google')}>
                    <div>
                        Signup/Login
                    </div>
                </span>
            }
            <span className={styles.dropDownMenuButton} onClick={() => modalStates('appearance')}>
                <div>
                    <Wrench />
                    Appearance
                </div>
            </span>
            <span className={styles.dropDownMenuButton} onClick={() => modalStates('stats')}><div>Your Stats</div></span>
